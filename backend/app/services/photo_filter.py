...
class PhotoFilter:
    ...
    async def filter(self, photos: List[Dict[str, Any]], user_id: str, photo_map: dict = None) -> List[Dict[str, Any]]:
        """
        过滤照片
    ...
    async def _filter_videos(self, photos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        过滤视频类型
    ...
    async def _filter_gemini_incompatible(self, photos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        过滤Gemini无法处理的图片
    ...
    async def _filter_duplicates(self, photos: List[Dict[str, Any]], photo_map: dict = None) -> List[Dict[str, Any]]:
        """
        过滤重复照片
                        ...
                        try:
                            ...
                            if hasattr(original_photo, 'duplicate_group'):
                                ...
                                if duplicate_group in duplicate_groups:
                                    ...
                                    logger.debug(f"根据iCloud重复组过滤掉照片: {photo.get('filename', '')}")
                                else:
                                    # 新的重复组，保留第一张
                                ...
                                if not is_duplicate:
                                    ...
                                    photo["is_duplicate"] = True
                                    logger.debug(f"根据iCloud重复标记过滤掉照片: {photo.get('filename', '')}")
                        except Exception as e:
